...
it.each(['unpinned', 'pinned', 'pinned-during-proof'])('reuses management tabs and respects pinning (%s)', async mode => {
  ...
  const tabs = [{ id: 7, url: `https://chatgpt.com/plugins?cos-plugin-refresh=${id}#settings/Plugins`, pinned: false }, { id: 8, url: 'https://chatgpt.com/c/user-conversation', pinned: false }];
  const create = vi.fn(async () => ({ id: 9 }));
  const remove = vi.fn();

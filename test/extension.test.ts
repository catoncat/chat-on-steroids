...
it('bounds an unresponsive tab reply so browser maintenance can continue', async () => {
  vi.useFakeTimers();
  try {
    ...
    const tabReply = vm.runInNewContext(`${code}\ntabReply`, {
      setTimeout, clearTimeout, Promise,
      chrome: { tabs: { sendMessage: async () => neverReplies } }
    });
    let settled = false;
    const pending = tabReply(71, { type: 'probe' }, { documentId: 'doc' }).then((reply: unknown) => {
      settled = true;

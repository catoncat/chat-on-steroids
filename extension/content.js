    ...
    for (let index = 0; index < answer.turns.length; index++) {
      ...
      for (const item of items) {
        if (item.type === 'activity') {
          ...
          if (owner) noteTurnProgress(owner);
          emit({
            kind: 'page_tool',
            text: activity.label,
  ...
  async function pullActivity() {
    ...
    try {
      ...
      if (resumeIdentityPending) {
        ...
        // Preserve the stronger reload identity fence added upstream: when the app also names
        // the stopped question, do not anchor the turn until that exact question is visible.
        const stopReady = !data.stopTurn || (data.stopTurn.turnId === appActiveTurnId && stopQuestionMatches(data.stopTurn.userMessageId));
        if (stopReady) {
          unrecordedGeneratingSince = 0;
          adoptOpenTurn(appActiveTurnId, data.stopTurn?.userMessageId ?? null);
        }
      }
